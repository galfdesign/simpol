...
const PRESETS_SCREED = [
  ...
  { id: "high", name: "Теплопроводная смесь", k: 2.00 },
];
...
const PRESETS_UNDERLAY = [
  ...
  { id: "bubble10", name: "Фольга с пузырьками 10 мм", type: "bubble", t: 0.010, eps: 0.05 },
...
export default function App() {
  ...
  const [screedThk, setScreedThk] = useState(0.070);
  ...
  const [loopPosFrac] = useState(0.5); // фиксируем центр
  ...
  const debounced = useDebounce({ Tair, Ts, Tr, spacing, pipeOD, screedThk, hTop, belowT, airVel: airVelSmooth, screed, cover, under, insul, nPipes, layout, loopLength, loopPosFrac, useFixedArea, areaM2, fixScale, autoReturn, flowLpm }, 150);
  ...
  const results = useMemo(()=>{
    const { Tair, Ts, Tr, spacing, pipeOD, screedThk, hTop, belowT, airVel, screed, cover, under, insul, nPipes, layout, loopLength, loopPosFrac, useFixedArea, areaM2, fixScale, autoReturn, flowLpm } = debounced;
    ...
    // Подложка: эквивалентная теплопроводность для пузырьковой фольги (воздушный зазор + низкоэмисс. поверхность)
    let tUnder = 0, kUnder = 1;
    if (under.type === 'bubble' && under.t > 0){
      tUnder = under.t;
      const tGap = under.t;
      // Консервативная учебная модель по ISO 6946:
      // базовое R для невентилируемого горизонтального зазора, тепло вниз (~0.11 м²K/Вт на 10 мм)
      const Rair_base = 0.11 * (tGap/0.01);
      // усиление за счёт низкой эмиссии фольги (без удвоения радиации): 1.5–1.7 для ε≈0.05
      const factor = 1.6;
      const Rgap = Rair_base * factor;
      kUnder = Math.max(0.002, tGap / Rgap);
    } else if (under.type === 'foil'){
      tUnder = 0; // чистая фольга в контакте — эффекта нет
      kUnder = 1;
    }
    ...
    function computePipeTemperatures(n, Ts, Tr, layout, Tair, L, x, S){
      ...
      if (layout === 'meander') {
        // Змейка: последовательное убывание слева направо
        const start = (L * 0.5) - ((n - 1) * deltaAdj) / 2;
        return Array.from({ length: n }, (_, k) => {
          const s = Math.max(0, Math.min(L, start + k * deltaAdj));
          return Tair + (Ts - Tair) * Math.exp(-alpha * s);
        });
      }
      // Спираль: падение через трубу (чередование горячая/холоднее/горячая ...)
      // even (0,2,...) немного ближе к подаче, odd (1,3,...) — ближе к обратке
      return Array.from({ length: n }, (_, k) => {
        const pairIndex = Math.floor(k / 2);
        const offset = (0.5 + pairIndex) * deltaAdj;
        const s = (k % 2 === 0) ? (L * 0.5 - offset) : (L * 0.5 + offset);
        const sClamped = Math.max(0, Math.min(L, s));
        return Tair + (Ts - Tair) * Math.exp(-alpha * sClamped);
      });
    }
    const L_eff = Math.max(5, useFixedArea ? (Math.max(1e-3, areaM2) / Math.max(0.02, S)) : loopLength);
    const xPos = 0.5 * Math.max(1e-6, L_eff);
    ...
    const Qloop = qTotalPerArea * W * L_eff; // Вт (Дж/с)
    const Tr_auto = Ts - Qloop / Math.max(1e-6, (mdot*cp));

    return {
      img, overlays, widthPx, heightPx, profile,
      metrics: {
        Tavg: +Tavg.toFixed(2), Tmax: +TmaxSurf.toFixed(2), Tmin: +TminSurf.toFixed(2),
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
        <div className="grid grid-cols-3 items-center">
          ...
          <a
            ...
            aria-label="Telegram"
            className="justify-self-end text-[#24A1DE] hover:text-[#1b8dc6] transition-colors"
          ...
          <div className="lg:col-span-1 rounded-2xl border p-4 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              {(()=>{ const val = +flowLpm.toFixed(2); return (
                <SliderField label="Расход контура, л/мин" min={0.5} max={6} step={0.1} value={val} onChange={(x)=> setFlowLpm(x)} />
              )})()}
            </div>
            ...
            <div className="rounded-2xl border p-4 shadow-sm mt-4">
              ...
              <div className="grid grid-cols-5 gap-3 items-center">
                ...
                {/* Используем фиксированную площадь 9 м², длина считается автоматически от шага */}
                <SliderField
                  label="Позиция среза (фикс. центр)"
                  hint={"Срез берётся по центру петли (x=L/2). Ползунок отключён."}
                  min={0}
                  max={0.5}
                  step={0.01}
                  value={loopPosFrac}
              ...
              <div className="rounded-2xl overflow-hidden border shadow-sm relative">
                ...
                <div className="text-xs text-gray-600 mt-2">
                  <ul className="list-disc ml-5 space-y-1">
                    <li>Пунктир — границы слоёв. Круг — труба (сечение ячейки шага).</li>
                    <li>Цвет: синий→красный, авто-масштаб.</li>
                    <li>Моделируется один шаг труб с симметрией по краям.</li>
                  </ul>
                </div>
              </div>
